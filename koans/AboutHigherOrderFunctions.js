...
describe("About Higher Order Functions", function() {
  ...
  it("should use filter to return array items that meet a criteria", function () {
    var numbers = [1,2,3];
    var odd     = _(numbers).filter(function(x) { return x % 2 !== 0 }); //filter function returns odd
    
    expect(odd).toEqual([1, 3]);
    expect(odd.length).toBe(2);
    expect(numbers.length).toBe(3); //doesn't manipulate the numbers array
  });
  ...
  it("should use 'forEach' for simple iteration", function() {
    ...
    //numbers.forEach(function(iten){...})
    //msg += even elements
    expect(msg).toEqual('falsetruefalse'); //1 === 0 => false, 0 === 0 => true, 1 === 0 => false 
    expect(numbers).toEqual([1, 2, 3]);
  });
  ...
  it("should use range to generate an array", function() {
    expect(_.range(3)).toEqual([0, 1, 2]); //first three numbers from 0 
    expect(_.range(1, 4)).toEqual([1, 2, 3]); //not include last number 
